...
import it.polito.policytool.ui.view.ComparisonResult.ResultComparator;
...
public class ComparisonResultView  extends ViewPart{
	
	public static final String ID = "it.polito.policytool.ui.view.ComparisonResult.ComparisonResultView";
	
	private Set<Semilattice<GenericRule>> result;
	...
	private ComparisonResultView		thisView;

	private Composite				parent;
	
	public ComparisonResultView() {
		PolicyToolUIModel.getInstance().setComparisonResultView(this);
	}
	...
	public void createPartControl(Composite parent) {
		...
		this.setPartName("Comparison Results");
	...
	public void setInput(Set<Semilattice<GenericRule>> result) {
		this.result = result;
		createControl();
		HashSet<GenericRule> rule_result = new HashSet<>();
		for(Semilattice<GenericRule>  sl:result){
			try{
				rule_result.addAll(sl.getOutgoingAdjacentVertices(sl.getRoot()));
				} catch (Exception e){
			}
		}
		tableViewer.setInput(rule_result);
	}
