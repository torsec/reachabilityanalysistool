...
public class StandardRegExpSelector implements RegExpSelector, Cloneable {
	...
	Automaton automata; 
	RegExp regex;
	private static String selName="Standard Regex";
...
//		this.regex = ;
		...
		else automata = automata.union(r.toAutomaton());
	...
	public void intersection(Selector s) throws IllegalArgumentException {
		...
		automata = BasicOperations.intersection(automata, ((StandardRegExpSelector) s).automata);
	...
	public void union(Selector s) throws IllegalArgumentException {
		...
		automata = BasicOperations.union(automata, ((StandardRegExpSelector) s).automata);
	...
	public void setMinus(Selector s) throws IllegalArgumentException {
		...
		if (isFull()){
			automata = BasicOperations.complement(((StandardRegExpSelector)s).automata);
			regex=null;
			state = RegexState.VALUE;
			return;
		}
		
		automata = BasicOperations.minus(automata, ((StandardRegExpSelector) s).automata);
	...
	public boolean isIntersecting(Selector s) throws IllegalArgumentException {
		...
		return OtherOperations.intersecting(automata, ((StandardRegExpSelector)s).automata);
	...
	return OtherOperations.equivalent(automata, ((StandardRegExpSelector)s).automata);
	}

	@Override
	public boolean isSubset(Selector s) throws IllegalArgumentException {
		//TODO: Vedere con aldo questa parte teorica. L'insieme vuoto subset di ogni set, 
		//una regola con un campo vuoto ha senso? nn matcha mai quindi a che serve?
		...
		return OtherOperations.subsetNotEquivalent(automata, ((StandardRegExpSelector)s).automata);
	}
	...
	public boolean isSubsetOrEquivalent(Selector s) throws IllegalArgumentException {
		...
		return BasicOperations.subsetOf(automata, ((StandardRegExpSelector)s).automata);
