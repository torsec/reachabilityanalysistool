...
public class TimeSelector extends TotalOrderedSelectorImpl {
	...
	public static String [] abb={"S","M","T","W","H","F","A"};
//	private static String [] days={"Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"};
	...
	private static int maxTime = 10079;
	private static String selName="Time";
	...
	private static String convert(int timeS, int timeE){
		...
		int dayE = timeE/(24*60);
		int minE = timeE%(24*60);
		int hourE = minE/60;
		minE = minE%60;
		
		if (minS==0 && hourS==0 && minE==0 && hourE==24)
			return ""+dayS;
	...
	private static int[] convert(String time) throws InvalidRangeException{
		...
		if (st.hasMoreTokens()){
			...
			if (st2.hasMoreTokens()){
				...
				result[1]=day*(24*60)+eh*60+em;
			} else {
				result[1]=-1;
			}
			
			result[0]=day*(24*60)+sh*60+sm;
			
		} else {
			result[0]=day*(24*60);
			result[1]=(day+1)*(24*60);
		}
