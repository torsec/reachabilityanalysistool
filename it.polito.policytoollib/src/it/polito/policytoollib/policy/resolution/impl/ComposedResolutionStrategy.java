 ...
 */
public class ComposedResolutionStrategy extends
		ResolutionStrategy {
	 ...
	 *            policy is composed
	 */
	public ComposedResolutionStrategy(
			LinkedList<LinkedList<Policy>> policy_list,
			List<Policy> org_policy_list) {
		...
		for (LinkedList<Policy> p_list : policy_list) {
			...
			for (Policy p : p_list) {
				if (rp.get(p).isEmpty())
					...
					a_list[ii++] = p.getResolutionStrategy().composeActions(
							rp.get(p));
			}
	...
	private Action getSerialAction(Action[] actionList) {
		...
		for (Action action : actionList) {
			if (action instanceof IPSecAction
					|| action instanceof IPSecActionSet) {
				...
				if (action instanceof IPSecAction) {
					...
					ipsecactionlist = ((IPSecActionSet) action)
							.getSecActionList();

				for (IPSecAction a : ipsecactionlist) {
					if (a.getType() == IPSecActionType.AH
							|| a.getType() == IPSecActionType.ESP) {
						IPSecActionStack.push(a);
					}

					if (a.getType() == IPSecActionType.INVERT_AH
							|| a.getType() == IPSecActionType.INVERT_ESP) {
