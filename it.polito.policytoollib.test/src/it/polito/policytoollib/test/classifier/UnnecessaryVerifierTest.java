...
public class UnnecessaryVerifierTest {

	@Test
	public void isUnnecessary() throws IncompatibleExternalDataException, DuplicateExternalDataException, InvalidRangeException, UnsupportedSelectorException {
		...
		// --------------------Rule5
		LinkedHashMap<String, Selector> ss5 = new LinkedHashMap<String, Selector>();
		PortSelector ps51 = new PortSelector();
		ss5.put("PS1", ps51);
		ps51.addRange(15, 30);
		PortSelector ps52 = new PortSelector();
		ss5.put("PS2", ps52);
		ps52.addRange(15, 30);
		ConditionClause cc5 = new ConditionClause(ss5);
		GenericRule rule5 = new GenericRule(FilteringAction.ALLOW, cc5, "R5");
		policyImpl.insertRule(rule5, 5);
		...
		Assert.assertFalse(policyImpl.getRuleClassifier().isUnnecessary(rule4, rule_list1));
		...
		Assert.assertTrue(policyImpl.getRuleClassifier().isUnnecessary(rule4, rule_list2));
	}
	
	@Test
	public void isUnnecessary1() throws IncompatibleExternalDataException, DuplicateExternalDataException, InvalidRangeException, UnsupportedSelectorException, NoExternalDataException, DuplicatedRuleException, UnmanagedRuleException {
		PolicyImpl policyImpl = new PolicyImpl(new FMRResolutionStrategy(), FilteringAction.DENY, PolicyType.FILTERING, "Test");
		...
		GenericRule[] rule_list = new GenericRule[1];
		
		rule_list[0] = rule1;
		Assert.assertTrue(policyImpl.getRuleClassifier().isUnnecessary(rule2, rule_list));
		Assert.assertTrue(policyImpl.getRuleClassifier().isHidden(rule2, rule_list));
		
		rule_list[0] = rule2;
		Assert.assertFalse(policyImpl.getRuleClassifier().isUnnecessary(rule1, rule_list));
		Assert.assertFalse(policyImpl.getRuleClassifier().isHidden(rule1, rule_list));
